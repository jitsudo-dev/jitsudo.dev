...
2. **Start the local environment**
   ...
   Setting `JITSUDO_SERVER` tells the CLI where to find the control plane. You can also pass `--server http://localhost:8080` on individual commands, or set `server: http://localhost:8080` in `~/.jitsudo/config.yaml`.
